...
sacrebleu -t ${CORPUS} -l ${SRC}-${DEST} --echo src > wmt14.en-fr.en
START_TIME=$SECONDS
cat wmt14.en-fr.en| $NORM_PUNC -l en | $TOKENIZER -a -l en -q | python $BPEROOT/apply_bpe.py -c ${CODE} | fairseq-interactive data-bin/${MODEL} --path data-bin/${MODEL}/model.pt --remove-bpe --buffer-size 1024 --batch-size 16 -s ${SRC} -t ${DEST} | grep -P "^H" |cut -f 3- | $DETOKENIZER -l ${DEST} -q > ${CORPUS}.test.${SRC}-${DEST}.${SRC}.out
